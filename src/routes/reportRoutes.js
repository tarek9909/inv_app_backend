...
const reportLimiter = rateLimit({ windowMs: 60 * 1000, max: 15, message: { success: false, message: 'Too many report requests. Please wait.', errors: [] } });
...
router.get('/driver-payroll', authenticate, requirePermission('reports.view'), reportLimiter, validate(schemas.reportQuery, 'query'), controller.driverPayroll);
...
router.get('/drivers/detail', authenticate, requirePermission('reports.view'), reportLimiter, validate(schemas.driverReportQuery, 'query'), controller.driverDetailReports);
...
router.get('/drivers/:id/detail', authenticate, requirePermission('reports.view'), reportLimiter, validate(schemas.reportQuery, 'query'), controller.driverDetailReport);
